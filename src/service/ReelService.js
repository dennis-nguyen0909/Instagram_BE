    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!reel.likes.includes(idUser)) {
          const like = await Reel.findByIdAndUpdate(
            idReel,
            {
          ...
          if (like) {
            const likeReel = await Reel.find()
              .sort({ createdAt: -1 })
          ...
          const unLike = await Reel.findByIdAndUpdate(
            idReel,
            {
              $pull: { likes: idUser },
            },
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!reel) {
          ...
          const commentReel = await Reel.findByIdAndUpdate(
            idReel,
            {
